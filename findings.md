...
- `$C75D/$C75E/$C761/$C764/$C774` are the drop-input column swap animation state. `StartDropAnim` stores the selected column and seeds two seven-entry cascade arrays; `AnimateDropping` advances them every two frames, blocks new drops while active, and clears `DROP_ANIM_ACTIVE` after swapping the selected `$C66A` column-state bytes.
