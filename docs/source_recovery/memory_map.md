...
| `$C66A-$C66D` | `COLUMN_TOP_ROWS` | High | Four per-column row/fall-target bytes. Seeded from level setup, indexed by `PIECE_ROTATION`, used by drawing and fall logic, and swapped by `AnimateDropping`; `BOARD_FALL_END_ROW` (`$0F`) is the clear/end boundary. |
...
| `$C699` | `COLUMN_TOP_ROW_SEED` | High | Loaded from `LevelCountTable` or forced to `BOARD_FALL_END_ROW`; `SeedColumnTopRows` copies it into all four `COLUMN_TOP_ROWS` entries, and `ProcessInputTitle` uses it to size the initial board-fill loop. |
...
| `$C6CF` | `LEVEL_DISPLAY_ONES` | High | Initialized from `OPTION_LEVEL + 1` or `LINK_2P_SELECTED_LEVEL + 1`, incremented as a decimal ones digit by `AdvanceATypeLevelDisplayDigits` / `AdvanceLevelDisplayDigits`, drawn by `UpdateLevel` / `DrawLevelDisplayDigits`, and copied into result records. |
...
| `$C6D1` | `LEVEL_DISPLAY_TICK_COUNTER` | High | `TitleInputHandler` increments this divider and advances `LEVEL_DISPLAY_ONES` / `LEVEL_DISPLAY_TENS` every 10 ticks; setup paths clear it through `ClearLevelDisplayTickCounter`. |
...
| `$C6E6` | `LINK_FIELD_EVENT_PAYLOAD` | Medium | Falling-piece code builds a bit-6 field-event payload here with `LINK_FIELD_EVENT_FLAG` (`$40`), then the round-complete path copies it into `LINK_SEND_QUEUE_0`. |
...
| `$C703` | `RESULT_CLEAR_FLAG` | High | B-type board-clear detection sets this when all four `COLUMN_TOP_ROWS` reach `BOARD_FALL_END_ROW`; result display checks it before drawing the clear/win message and setup paths clear it for a new round/playfield. |
...
| `$C74B-$C755` | `CURRENT_RESULT_RECORD` | High | Staged result record built from `SCORE_DIGITS`, level display digits, and either egg digits or `TOTAL_TIMER_DIGITS` before being inserted into the A/B record table. |
